...
pub fn init_repo(target_path: &Path) -> Result<()> {
    run_git(Some(target_path), &["init"]).context("git init failed")
}

pub fn add_all(target_path: &Path) -> Result<()> {
    run_git(Some(target_path), &["add", "-A"]).context("git add -A failed")
}
